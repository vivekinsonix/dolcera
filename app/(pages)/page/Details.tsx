      ...
      <div className="mt-4 flex justify-center gap-2">
        {images.map((_, idx) => (
          <button
            ...
            className={`h-2 rounded-full transition-all ${current === idx ? 'w-6 bg-gray-900 dark:bg-white' : 'w-2 bg-gray-400'}`}
          />
        ))}
      </div>
    </div>
...
const Details = ({ slug }: { slug: Record<string, string> }) => {
  ...
  const seoData = useMemo(() => {
    ...
    return {
      ...
      seoKeywords: seo?.keywords || 'About Insonix, Insonix Services',
