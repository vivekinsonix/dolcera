...
const Icon = React.memo(({ svg }: { svg: string }) => (
  <div aria-hidden="true" className="[&_svg]:w-6 [&_svg]:h-6" dangerouslySetInnerHTML={{ __html: svg }} />
));
...
const View3_block = ({ data, loading }: { data: any; loading: boolean }) => {
  ...
  return (
    <>
      ...
      {!loading && (
        <div className="container prose prose-headings:mt-0  mx-auto max-w-7xl  py-0 md:py-0 dark:text-white">
          {view_name && <h2 className="dark:text-white">{view_name}</h2>}
          ...
          {/* SECOND CARD */}
          {card_2_description && (
            <div className="p-0 my-5 dark:bg-primary">
              {card_2_title && (
