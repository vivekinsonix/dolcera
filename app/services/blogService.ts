...
export function getBlogsIds() {
  return apiClient
    ...
    .then((res) => res.data?.data?.map((blog: any) => blog?.slug));
}

export function getPaginatedBlogs(pageno = 1, records = 5) {
  return apiClient
    .get(`/blogs?&&populate=*&sort=createdAt:desc&pagination[page]=${pageno}&pagination[pageSize]=${records}`)
    .then((res) => res.data);
}
...
export async function getBlogBySlug(slug: string) {
  ...
  const requests = [
    apiClient.get('/blogs', { params: { ...baseParams } }),
    apiClient.get('/blogs', { params: { ...baseParams, status: 'draft' } }),
  ];
