...
--------------------------------------------------------- */
const SkeletonMAIN = () => (
  <section className="relative isolate overflow-hidden pt-16 animate-pulse">
...
export const MAIN = ({ content, loading }: { content: any; loading: boolean }) => {
  ...
  return (
    <>
      ...
      {/* --------------------- HERO SECTION ------------------ */}
      <section className="relative isolate overflow-hidden pt-16 dark:bg-primary">
        {/* Background image */}
        {imageUrl ? (
        ...
        <div className="mx-auto container md:px-0 px-4  dark:text-white  py-8">
          {/* Title */}
          {title && <h1 className="max-w-3xl text-3xl leading-tight font-bold md:text-5xl">{title}</h1>}

          {/* Subtitle */}
          {sub_title && <p className="mt-4 max-w-2xl text-lg dark:text-white/90">{sub_title}</p>}
