  ...
  return (
    <header role="banner" className="relative isolate overflow-hidden pt-4 px-4">
      ...
      <div className="relative z-10 container mx-auto  py-10 sm:py-24 md:py-32 text-left dark:text-white">
        {/* Title */}
        {loading ? (
          <div className="h-10 w-1/2 rounded bg-gray-600 animate-pulse" />
        ) : (
        ...
        {/* Description */}
        {loading ? (
          <div className="mt-4 h-4 w-2/3 rounded bg-gray-600 animate-pulse" />
        ) : (
          description && <p className="mt-4 text-lg dark:text-white/90">{description}</p>
        )}
          ...
          buttons.length > 0 && (
            <div className="mt-8 flex flex-wrap gap-4">
              {buttons.map((btn, i) => {
                ...
                let variantClass =
                  'inline-block bg-blue-100 text-black font-bold py-3 px-8 rounded-full uppercase tracking-wide hover:scale-105 transition';
